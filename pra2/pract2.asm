...
DATOS SEGMENT
    MATRIZ     DB 2, 3, 5, 7, 11, 13, 11, 7, 5
    TEMP       DB ?
    SALIDA     DB 10, 6 DUP(30H), 13, 10, '$'
    NUMERO     DB 4, 6 DUP(0)
    ASCIIMX    DB 10, 6 DUP(' '), "|             |", 13, 10,"|A| = |             | =        ", 13, 10, 6 DUP (' '), "|             |$"
    ...
    INITMSG    DB "INTRODUCE 1 O 2 PARA ESCOGER OPCION:",13,10,"1) CALCULAR DETERMINANTE CON VALORES POR DEFECTO",13,10, "2) CALCULAR DETERMINANTE CON VALORES POR TECLADO",13,10,'$'
    OPTION     DB 2, 4 DUP (0)
    CLR_PANT   DB 1BH,"[2","J$"
DATOS ENDS
;**************************************************************************
...
PORDEFECTO:
MOV AL, MATRIZ[0] ; CALCULA EL DETERMINANTE
MOV AH, MATRIZ[4]
MOV BH, MATRIZ[8]
CALL IMUL3
ADC CX, AX
...
MOV BX, 5 ; IMPRIME EL RESULTADO EN SU POSICION DE ASCIIMX
MOV SI, OFFSET ASCIIMX
ADD SI, 48
CALL TOASCII

MOV BX, 3 ; BLOQUES DE IMPRESION EN ASCIIMX LOS VALORES DE CADA ELEMENTO
MOV CH, MATRIZ[0]
MOV CL, 8
SAR CX, CL
MOV SI, OFFSET ASCIIMX
...
MOV DX, OFFSET ASCIIMX ; IMPRIME ASCIIMX POR PANTALLA
MOV AH, 9
INT 21H
