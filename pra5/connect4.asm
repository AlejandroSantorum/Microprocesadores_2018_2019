...
INICIO PROC FAR
    ...
    MOV DX, OFFSET BOARD
    CALL PRINTF
    ; RESET COUNTER AND TIME_FLAG
    MOV AX, 0
	MOV ES, AX
	LES BX, ES:[70h*4]
	CLI
    MOV WORD PTR ES:[BX-4], 0 ; COUNTER = 0
    MOV WORD PTR ES:[BX-6], 0 ; TIME_FLAG = 0
	STI
    ; CHECKING TURN
    CMP TURN, 0
    JE TURN1
	...
	INT 21H
    GET_COL:
    ; CHECKING IF TIME_FLAG IS SET (FLAG THAT INDICATES IF 5 SECONDS HAVE PASSED)
	MOV AX, 0
	MOV ES, AX
    LES BX, ES:[70H*4]
	CLI
    ...
    INT 1AH
    MOV AX, DX
    MOV DX, 0 ; PREVENTING OVERFLOW
    ; DIVIDING TIME BY 7, TO GET A NUMBER IN [0,6] RANGE
    MOV BX, 7
    DIV BX
    MOV AL, DL ; REMAINDER IN DX, WE'RE JUST INTERESTED ON LOW PART
    ; CHECKING THE COL IS AVAILABLE
    CALL CHECK_COL
    CMP AH, 0 ; IF 0 => NOT AVAILABLE => GETTING ANOTHER RANDOM INDEX
    JE MOVE_RANDOMLY
    ; INSERTING PIECE
    JMP INSERT_PIECE_LABEL
